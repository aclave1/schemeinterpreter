public class Keywords {
    public static final String DEFINE = "define";
    public static final String LAMBDA = "lambda";
    public static final String IF = "if";
    public static final String COND = "cond";
    public static final String LET = "let";
    public static final String BEGIN = "begin";
    public static final String QUOTE = "\'";
    public static final String QUOTE_WORD = "quote";
    public static final String SET = "set!";
    public static final String LPAREN = "(";



    //binary arithmetic
    public static final String BINARY_PLUS = "b+";
    public static final String BINARY_MINUS = "b-";
    public static final String BINARY_MULT = "b*";
    public static final String BINARY_DIV = "b/";
    public static final String BINARY_EQUALS = "b=";
    public static final String BINARY_LT = "b<";
    //list operations
    public static final String CAR = "car";
    public static final String CDR = "cdr";
    public static final String CONS = "cons";
    public static final String SET_CAR = "set-car!";
    public static final String SET_CDR = "set-cdr!";
    //type checks
    public static final String NUMBERCHECK = "number?";
    public static final String SYMBOLCHECK = "symbol?";
    public static final String NULLCHECK = "null?";
    public static final String PAIRCHECK = "pair?";
    public static final String EQCHECK = "eq?";
    public static final String PROCEDURECHECK = "procedure?";
    //io
    public static final String IOREAD = "read";
    public static final String IOWRITE = "write";
    public static final String IODISPLAY = "display";
    public static final String IONEWLINE = "newline";
    public static final String EVAL = "eval";
    public static final String APPLY = "apply";
    public static final String INTERACTION_ENVIRONMENT = "interaction-environment";
}
